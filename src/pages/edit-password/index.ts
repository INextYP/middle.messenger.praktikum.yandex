...
> {
    ...
    componentDidMount() {
        ;(this.attributes.form as EditPasswordPageKeys).form.addEventListener(
            'submit',
            (e: Event) => {
                e.preventDefault()

                const { id: oldPasswordInputId, value: oldPasswordInputValue } =
                    this.getInputValue(
                        'old_password',
                    ) as unknown as HTMLInputElement
                const { id: newPasswordInputId, value: newPasswordInputValue } =
                    this.getInputValue(
                        'new_password',
                    ) as unknown as HTMLInputElement
                const {
                ...
                const [isValidOldPasswordInput, oldPasswordInputErrorMessage] =
                    validator.validate(
                        'password' as InputType,
                        oldPasswordInputValue,
                    ) as ValidationReturnType

                const [isValidNewPasswordInput, newPasswordInputErrorMessage] =
                    validator.validate(
                        'password' as InputType,
                ...
                if (!isValidOldPasswordInput) {
                    const key = oldPasswordInputId as keyof EditPasswordPageKeys
                    ;(
                ...
                if (!isValidCheckNewPasswordInput) {
                    const key = newPasswordInputId as keyof EditPasswordPageKeys
                    ;(
                        this.attributes?.[key] as AttributesType
                    ).attributes.errorText.setProps({
